    ...
    nv.models.candlestickBar = function () {
        ...
        function chart(selection) {
            selection.each(function (data) {
                ...
                var text = tickGroups.append('text')
                    .attr('class', 'nv-candlestick-text')
                    .attr('transform', function (d, i) { return 'translate(' + (x(getX(d, i)) - barWidth/2) + ','+(y(getHigh(d, i)) -50)+')'; })
                    .text('BBandSE')
                    .attr('visibility', function(d, i){ return i === 17 ? 'visible' : 'hidden' })
                    .style('stroke', 'red')
                    .style('font-size', '14px')

                var polyline = tickGroups.append('polyline')
                    .attr('class', 'nv-candlestick-polyline')
                    .attr('transform', function (d, i) { return 'translate(' + x(getX(d, i)) + ','+
                    (y(getY(d, i)) - (getOpen(d, i) > getClose(d, i) ? (y(getClose(d, i)) - y(getOpen(d, i))) : 0))+')'; })
                    .attr('points', '15,2  30,-10  30,10  15,2')
                    .style('stroke', '006600')
                    .style('fill', 'red')
                    .attr('visibility', function(d, i){ return i === 17 ? 'visible' : 'hidden' })

                var image = tickGroups.append('image')
                    .attr('class', 'nv-candlestick-image')
                    .attr('transform', function (d, i) { return 'translate(' + (x(getX(d, i)) -10 ) + ','+(y(getHigh(d, i)) -40)+')'; })
                    .attr('x', 0)
                    .attr('y', 0)
                    .attr('width', 20)
                    .attr('height', 30)
                    .attr('xlink:href', 'bower_components/nvd3/js/arrow-down.gif')
                    .attr('visibility', function(d, i){ return i === 17 ? 'visible' : 'hidden' });
                ...
                container.selectAll('.nv-candlestick-text').transition()
                    .attr('class', 'nv-candlestick-text')
                    .attr('transform', function (d, i) { return 'translate(' + (x(getX(d, i)) - 30) + ','+(y(getHigh(d, i)) -50)+')'; })
                    .text('BBandSE')
                    .attr('visibility', function(d, i){ return i === 17 ? 'visible' : 'hidden' })
                    
                container.selectAll('.nv-candlestick-polyline').transition()
                    .attr('class', 'nv-candlestick-polyline')
                    .attr('transform', function (d, i) {
                        return 'translate(' + x(getX(d, i)) + ','
                            + (y(getY(d, i)) - (getOpen(d, i) > getClose(d, i) ? (y(getClose(d, i)) - y(getOpen(d, i))) : 0)) + ')';
                    })
                    .attr('points', '15,2  30,-10  30,10  15,2')
                    .style('stroke', '006600')
                ...
                container.selectAll('.nv-candlestick-image').transition()
                    .attr('class', 'nv-candlestick-image')
                    .attr('transform', function (d, i) { return 'translate(' + ( x(getX(d, i)) - 10 ) + ','+(y(getHigh(d, i)) -40)+')'; })
                    .attr('x', 0)
                    .attr('y', 0)
